...
"""
- 먼저, 이 정리글을 보는 한가지 팁을 알려드리자면, 모든 예시와 설명을 외우려고 하지 마세요.
    되도록 핵심 부분을 정리하여 이해가 가는 방향으로 설명을 달았으므로,
    일단 이해가 되었으면 설명글은 넘어가시면 되며,
    코드의 경우는 한번 머릿속으로 결과를 시뮬레이션 해보시고 넘어가시면 됩니다.
    일단 그렇게 이해하고 넘어가신 후, 실제 원하시는 것을 만드실 때, 
    이전에 접해봤던 부분이라면 그 키워드로 챗 GPT 나 구글링으로 원하는 부분을 완성해나가면 됩니다.
    조금 더 손이랑 머리에 익히고 싶다면, 저와 같이 자신의 스타일로 글을 정리하고, 
    제가 미처 정리하지 못한 부분을 스스로 찾아서 정리하는 것도 매우 좋은 방법입니다.
